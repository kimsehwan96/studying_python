...
```
    ...
    쿠키를 지우기 전까지는 모두 같은 연결로 인식한다. 많은 사용자가 접속하는 서버라면 쿠키 데이터가 메모리에 계속 저장된다. 따라서 쿠키 만료 시간을 설정해주어야 한다.
    
    일반적으로 웹 서버는 데이터를 크게 두가지 형태로 분류해서 다룬다. 첫 번째는 임시데이터 -> 일정 기간이 지나면 메모리 할당을 해제하고 사용하지 않는다. 두번째는 영구 데이터 -> 데이터베이스에 저장해서 사용

    쿠키와 세션 ID의 경우는 첫 번째 형태에 해당하고, 두 값으로 접근 할 수 있는 정보는 영구적 데이터에 해당한다.

```python3

response = make_response(
    '새 연결 : sessionId={}'.format(new_session_id))
    #만료 시간 5초
    response.set_cookie('sessionId', new_session_id, max_age=5)
)

#response.set_cookie(<name>, <넣을 값>, <만료시간>)
```
